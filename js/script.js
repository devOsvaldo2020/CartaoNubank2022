...
function cardEffect(event) {
    ...
    const centerX = card.offsetLeft + (larguraCard / 2);
    const centerY = card.offsetTop + (alturaCard / 2);
    const positionX = event.clientX - centerX;
    const positionY = event.clientY - centerY;

    const rotateX = ((+1) * 25 * positionY / (alturaCard / 2)).toFixed(2);
    const rotateY = ((-1) * 25 * positionX / (larguraCard / 2)).toFixed(2);
